...
Deze transformatie houdt in dat de matrix $E$ getranslateerd wordt naar $[\bm{e}_2-\bm{e}_1;\bm{e}_3-\bm{e}_1;...;\bm{e}_n-\bm{e}_1]$ en het gemeten spectrum naar $\bm{x} - \bm{e}_1$. Na deze transformatie kan het \textit{Penrose-inverse} algoritme worden toegepast, zodat men de abundanties $a_2,a_3,...,a_n$ krijgt. De laatste abundantie $a_1$ kan gevonden worden door de eenheidssomvoorwaarde te eisen, zodat $a_1 = 1 - \sum{i=2}^{n} a_i$.
...
Het probleem bij het \textit{MESMA} algorimte is dat ontmengt moet worden in functie van elke deelverzameling van endmembers, voor elk materiaal.
...
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=\textwidth,trim=0 200 0 175 cm]{export_PMC_10_100_corr.pdf}
...
\end{subfigure} \rulesep
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=\textwidth,trim=0 200 0 175 cm]{export_PMC_10_100_PP.pdf}
...
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=\textwidth,trim=0 200 0 175 cm]{export_PMC_10_100_angle.pdf}
...
Zoals te zien in de figuren en voorspeld in sectie \ref{sec:smesma}, geeft het semilineair model een verbetering in looptijd ten opzichte van het multilineair model , terwijl er maar een kleine verhoging is voor de reconstructie-error.
