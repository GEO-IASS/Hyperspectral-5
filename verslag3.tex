...
Earth observation is the science of collecting information about the earth surface, without physically touching it. This is done by measuring and recording reflected or emitted electromagnetic radiation and processing, analysing it and applying the result is information. These electromagnetic waves are measured using a hyper spectral camera attached to a satellite. Particular interest goes to converting the spectra of the different pixels into a hyper spectral image of quantities or abundances of different materials. Earth observation has applications in various areas, such as agriculture, geology, forestry, hydrology, ecology, studying land formations, sea-ice, biomass.., observing other planets, monitoring oil spills and many other applications.

In this dissertation the method of linear unmixing often used in literature has been described as are two important problems related to it, i.e. non-linearity and variability. Non-linearity means that to mix different materials the spectra of the different materials can not be simply added,instead internal reflections have to be considered. variability means that not every material has a fixed spectrum and that most materials may have multiple spectra.  

nest two state-of the art methods from literature for non-linear spectral unmixing and endmember variability are mentioned.
First, the multi-linear model, a methode for non-lineair spectral unmixing is described\cite{mlinmix}. Secondly, two methods to solve variability are discussed, as described in\textit{Hyperspectral unmixing with endmember variability via alternating angle minimization}\cite{mesma}: \textit{Multiple endmember spectral mixture analysis}, and \textit{alternating angle minimization}. This second method is a computationally more efficient version of the first. 

Both variability and non-linearity have been studied extensively. Diverse correction can be made to the linear model. Few research has been done to combining both corrections, even if both often occur simultaneously. In complex object, such as vegetation or buildings, on one hand much internal reflection occurs, while there are also a number of variations in each material. An example of the last problem is that the underside of a tree leaf may have a different spectrum, or that different compositions of mortar may have been used in buildings. Therefore, a method accounting for both corrections must be developed.

This dissertation researches different methods to insert both correction into a system, and compares between the different methods applying both corrections. First a numerical control is used to unmix different simulated data, next the methods, and a couple of varients applied are validated using an experimental dataset: the Alina dataset. 

\newpage
\chapter*{Abstract}
\addcontentsline{toc}{chapter}{Abstract (Nederlands)}

Aardobservatie is de wetenschap van het verzamelen van informatie van het aardoppervlak zonder hiermee in contact te komen. Dit wordt gedaan door het meten en vastleggen van gereflecteerde of uitgezonden elektromagnetische straling en het verwerken, analyseren en toepassen van deze informatie. Deze elektromagnetische golven worden gemeten door een hyperspectrale camera op een satelliet. Specifiek is men ge\"intereseerd in het omzetten van de spectra van de verschillende pixels in het hyperspectrale beeld naar de hoeveelheden ofwel abundanties van verschillende materialen. 
Aardobservatie heeft toepassingen in verschillende gebieden, zoals landbouw, geologie, bosbouw, hydrologie, ecologie, het bestuderen van landformaties, zee-ijs, biomassa, het observeren van andere planeten, het opvolgen van olierampen en vele andere toepassingen. 
...
Daarna worden twee state-of-the-art methodes uit de literatuur beschreven voor niet-lineair spectrale ontmenging en endmember variabiliteit. Als eerste wordt het multilineair model beschreven. Dit is een methode voor niet-lineaire spectrale ontmenging, en wordt uitvoerig beschreven in: \textit{A multilinear mixing model for nonlinear spectral unmixing}\cite{mlinmix}. Als tweede worden twee methodes die endmember-variabiliteit in rekening brengen beschreven - deze worden behandeld in: \textit{Hyperspectral unmixing with endmember variability via alternating angle minimization}\cite{mesma}- namelijk \textit{Multiple endmember spectral mixture analysis}, en \textit{alternating angle minimization}, wat een computationeel effici\"enter alternatief is voor \textit{Multiple endmember spectral mixture analysis}.

Zowel variabiliteit en niet-lineariteit zijn uitvoerig bestudeerd in de literatuur, en verschillende correcties op het lineaire model kunnen worden doorgevoerd. Er is weinig onderzoek gedaan naar het combineren van beide correcties, hoewel  deze beide problemen vaak allebei tegelijkertijd voorkomen. In complexe objecten, zoals vegetatie en gebouwen, komt enerzijds veel interne reflectie voor, ten gevolge van hun ingewikkelde structuur, en anderzijds veel verschillende variaties op hetzelfde materiaal. Dit laatste gebeurt bijvoorbeeld doordat de boven- en onderkant van een boomblad een verschillend spectrum hebben, of dat er verschillende samenstellingen van cement gebuikt zijn in een gebouw. Daarom is het belangrijk om een methode te ontwikkelen dat met beide correcties rekening houdt.

Daarom is er een deze thesis onderzoek gedaan naar verscheidene methoden om deze beide correcties door te voeren in een systeem en wordt een vergelijking gemaakt tussen de verschillende methoden. Er wordt eerst een numerieke controle gebruikt voor het ontmengen van verschillende gesimuleerde data, en nadien worden deze methodes en een aantal varianten hierop gevalideerd  op een experimentele dataset: de Alina dataset\cite{Alina}.
...
Buiten deze twee toepassingen zijn er nog veel andere toepassingen, zoals bosbouw, hydrologie, ecologie, het bestuderen van landformaties, zee-ijs, biomassa, het observeren van andere planeten, het opvolgen van olierampen en vele andere toepassingen. 
...
\section{semilineair AAM} \label{sec:MAAM}
...
semilineair AAM & \ref{sec:MAAM} & \ref{al:MAAM} 
\end{tabular}
...
\hline
real multilineair & 941.5419 & 941.1282 & 945.4887 & 944.4081 & 901.0079 & 924.5870 \\
\hline
free multilineair & 942.7313 & 914.6477 & 947.9145 & 946.6204 & 908.2608 & 938.4770 \\
\hline
strict multilineair & 903.7542 & 823.6796 & 791.1329 & 792.1679 & 822.2177 & 791.3122 \\
\hline
strong multilineair & 929.7014 & 902.9595 & 892.1193 & 881.2452 & 835.1917 & 925.4355 \\
\hline
...
\hline
strict semilineair AAM   & 0.0922 & 0.1567 & 0.0980 & 0.1007 & 0.0805 & 0.1143 \\
\hline
strong semilineair AAM  & 0.0876 & 0.1564 & 0.0833 & 0.0829 & 0.1018 & 0.0906 \\
\hline
free semilineair AAM  & 0.1638 & 0.1477 & 0.1629 & 0.1713 & 0.1129 & 0.1618 \\
\hline
real semilineair AAM  & 0.1625 & 0.1473 & 0.1129 & 0.1147 & 0.1024 & 0.1377 \\
\hline
\end{tabular}
...
\hline
strict semilineair AAM & 0.0022 & 0.0020 & 0.0045 & 0.0058 & 0.0535 & 0.0077 \\
\hline
strong semilineair AAM  & 0.0022 & 0.0020 & 0.0014 & 0.0021 & 0.0078 & 0.0070 \\
\hline
free semilineair AAM & 0.0021 & 0.0020 & 0.0013 & 0.0021 & 0.0077 & 0.0066 \\
\hline
real semilineair AAM & 0.0022 & 0.0020 & 0.0045 & 0.0058 & 0.0535 & 0.0070 \\
\hline
\end{tabular}
...
\hline
strict semilineair AAM & 0.0268 & 0.8960 & 1.0000 & 1.0000 & 1.0000 & 1.0000 \\
\hline
strong semilineair AAM & 0.0268 & 0.8960 & 0.3060 & 0.3631 & 0.1460 & 0.9049 \\
\hline
free semilineair AAM & 0.0256 & 0.8773 & 0.2946 & 0.3574 & 0.1447 & 0.8554 \\
\hline
real semilineair AAM & 0.0260 & 0.8935 & 1.0000 & 1.0000 & 1.0000 & 0.9056 \\
\hline
\end{tabular}
\caption{Reconstructie-error van de verschillende algoritmen voor verschillende pixels, gerenormaliseerd met de grooste error voor elke pixel. Deze zijn allen gekozen voor een pixel met $y=3$. Deze pixels bevatten voor de meerderheid respectievelijk: bos;bos en asfalt;asfalt;asfalt en voetpad; voetpad en grass; grass. Deze indeling is gemaakt op basis van wat zich werkelijk op de grond bevind, en kan bijgevolg verschillen van de gemeten abundanties.
\label{table:errosrn}}
\end{table}
...
\hline
strict semilineair AAM  & 0.1092 & 0.1187 & 0.1124 & 0.1010 & 0.0933 & 0.0944 \\
\hline
strong semilineair AAM  & 0.0891 & 0.0974 & 0.0930 & 0.0970 & 0.0963 & 0.0870 \\
\hline
free semilineair AAM  & 0.1520 & 0.1645 & 0.1659 & 0.1667 & 0.1568 & 0.1480 \\
\hline
real semilineair AAM  & 0.1274 & 0.1402 & 0.1264 & 0.1293 & 0.1090 & 0.1115 \\
\hline
\end{tabular}
...
\hline
strict semilineair AAM  & 0.0030 & 0.0026 & 0.0036 & 0.0093 & 0.0257 & 0.0202 \\
\hline
strong semilineair AAM  & 0.0029 & 0.0026 & 0.0019 & 0.0023 & 0.0055 & 0.0104 \\
\hline
free semilineair AAM  & 0.0028 & 0.0025 & 0.0018 & 0.0023 & 0.0053 & 0.0098 \\
\hline
real semilineair AAM  & 0.0029 & 0.0026 & 0.0036 & 0.0093 & 0.0257 & 0.0202 \\
\hline
\end{tabular}
